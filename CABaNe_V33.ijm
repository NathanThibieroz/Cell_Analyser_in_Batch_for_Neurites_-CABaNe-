...
function GUIMorpho(){
	...
	Dialog.addMessage("--------Thresholds--------");
		...
		Dialog.addToSameRow();
		Dialog.addChoice("Cell split using?", List_Type_Marker, Type_Marker);
...
var ChoiceBeforeAfter=newArray("Before", "After");
var BeforeAfter="Before";
	...
	Dialog.create("ImageSorter");
		...
		Dialog.addCheckbox("Shall images be automatically sorted in conditions folders?", UseWell);
		Dialog.addString("If yes, what character separate the conditions name from the rest?", ConditionSeparator);
		Dialog.addChoice("Is the condition name before or after the separator?", ChoiceBeforeAfter, BeforeAfter);
	Dialog.show();
	FolderImage=Dialog.getString();					
	FolderSaving=Dialog.getString();
	UseWell=Dialog.getCheckbox();
	ConditionSeparator=Dialog.getString();
	BeforeAfter=Dialog.getChoice();
...
//loop, open and process
	for(i=0; i<ListOfImages.length-(1+EnableThirdMarking); i++){
		...
		if(Cancel==0){
			print("Saving...");
			if(UseWell==1){
				PartsOfName= split(Image_without_tif[0], ConditionSeparator);
				if(BeforeAfter=="Before"){
					ImageWell= PartsOfName[0];
				}else {
					ImageWell= PartsOfName[1];
				}
